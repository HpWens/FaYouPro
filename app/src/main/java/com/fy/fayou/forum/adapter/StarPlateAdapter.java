...
public class StarPlateAdapter extends MeiBaseAdapter<PlateEntity> {

    OnItemListener mListener;

    public StarPlateAdapter(OnItemListener listener) {
        super(R.layout.item_forum_star_plate);
    ...
    protected void convert(@NonNull BaseViewHolder helper, PlateEntity item) {
        helper.setText(R.id.tv_name, getNonEmpty(item.nickName))
                .setText(R.id.tv_follow_num, helper.itemView.getResources().getString(R.string.forum_star_plate_comment_num, item.posts, item.comments));
